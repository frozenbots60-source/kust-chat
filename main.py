...
app = FastAPI(title="Kustify Ultra", description="Premium Animated Chat", version="6.1")
...
async def websocket_endpoint(websocket: WebSocket, group_id: str, user_id: str):
    # Accept immediately to avoid race conditions
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        #sidebar {
            ...
            z-index: 50; transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1);
        }
        ...
        .msg-avatar { width: 35px; height: 35px; border-radius: 50%; object-fit: cover; box-shadow: 0 2px 8px rgba(0,0,0,0.3); }
        ...
        #input-area {
            padding: 20px; background: rgba(9, 9, 11, 0.9); backdrop-filter: blur(10px);
        ...
        #msg-input {
            ...
            padding: 14px 24px; border-radius: 30px; color: white; font-size: 1rem; transition: 0.2s;
        }
        ...
        #vc-widget {
            ...
            display: none; flex-direction: column; transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
        }
        #vc-widget.minimized { width: 200px; top: auto; bottom: 20px; }
        ...
        #vc-widget.minimized .vc-grid, #vc-widget.minimized .vc-controls { display: none; }
        ...
        .vc-ring {
            ...
            opacity: 0; z-index: 1; transition: width 0.05s, height 0.05s, opacity 0.1s;
        }
        ...
        .modal-box {
            background: #18181b; width: 90%; max-width: 420px; padding: 30px;
    ...
    <script>
        ...
        function sendMessage() {
            ...
            state.ws.send(JSON.stringify({
                type: "message", user_name: state.user, user_pfp: state.pfp,
                text: txt, timestamp: Date.now()
            }));
            inp.value = '';
        }
        ...
        async function handleFile() {
            ...
            try {
                ...
                state.ws.send(JSON.stringify({
                    type: "message", user_name: state.user, user_pfp: state.pfp,
                    text: "", image_url: d.url, timestamp: Date.now()
                }));
            } catch(e) { alert("Err"); } 
        ...
        function joinVoice() {
            ...
            navigator.mediaDevices.getUserMedia(constraints).then(stream => {
                ...
                peer.on('open', () => { sendVc({type: "vc_signal", payload: {event: "join"}}); });
                peer.on('call', call => {
                    call.answer(stream);
        ...
        function handleVcEvent(d) {
            ...
            if(d.type === "vc_signal") {
                ...
                if(p.event === "ack") addVcUser(d.sender_id, d.sender_name, d.sender_pfp);
                if(p.event === "leave") removeVcUser(d.sender_id);
            }
            ...
            if(d.type === "vc_talking") {
                ...
                if(ring) {
                    ...
                    ring.style.width = `${50 * scale}px`; ring.style.height = `${50 * scale}px`;
                }
            }
        ...
        function removeVcUser(uid) {
            if(joinedUsers.has(uid)) { joinedUsers.delete(uid); const el = document.getElementById(`vc-u-${uid}`); if(el) el.remove(); }
        }
        ...
        function playStream(aud, s) {
            aud.srcObject = s;
            aud.addEventListener('loadedmetadata', () => { aud.play().catch(console.log); });
        ...
        function leaveVoice() {
            if(peer) { sendVc({type: "vc_signal", payload: {event: "leave"}}); peer.destroy(); }
            if(myStream) myStream.getTracks().forEach(t => t.stop());
            if(micLoop) clearInterval(micLoop); if(audioCtx) audioCtx.close();
        ...
        function toggleVcSize() { document.getElementById('vc-widget').classList.toggle('minimized'); }

        function dragElement(elmnt) {
            var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
            if (document.getElementById("vc-header")) {
                document.getElementById("vc-header").onmousedown = dragMouseDown;
                document.getElementById("vc-header").ontouchstart = dragMouseDown;
            } 
